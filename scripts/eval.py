...
def get_data_from_api(user_id, endpoint):
    ...
    '''
    url = f"http://web:5000{endpoint}/{user_id}"
    try:
...
if __name__=="__main__":
    #first challenge
    ground_truth_wiki=pd.read_csv('/assets/airports_wiki_sample.csv')
    '''
    Change File To test airport data cleaning 
    '''
    solution=pd.read_csv('/assets/airports_w_wiki.csv')
    ...
    #second challenge
    ground_truth_file = '/assets/ground_truth_sample.csv'  # specify your CSV file name
